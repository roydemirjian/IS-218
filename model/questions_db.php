...
class QuestionDB{
    ...
    public static function up_vote($answer_id){
        $db = Database::getDB();
        try {
            $sql = "UPDATE answers SET scores = scores + 1 WHERE id = '$answer_id' ";
            $q = $db->prepare($sql);
            $q->bindValue('id',$answer_id);
            $q->execute();
            $q->closeCursor();
        } catch(PDOException $e) {
            echo "Connection failed: " . $e->getMessage();
    ...
    public static function down_vote($answer_id){
        $db = Database::getDB();
        try {
            $sql = "UPDATE answers SET scores = scores - 1 WHERE id = '$answer_id' ";
            $q = $db->prepare($sql);
